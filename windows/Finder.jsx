...
import { useWindowNav } from '@/components/WindowNav'
...
const IconTile = ({ icon, name, selected, onClick, onDoubleClick }) => (
    <button
        type="button"
        onClick={onClick}
    ...
    >
        ...
        />
        <span className="text-center text-xs leading-tight text-white/90">{name}</span>
    </button>
)
...
const Finder = () => {
    ...
    const atMobileRoot = isMobile && folder === null
    const activeId = folder ?? 'work'
    const current = FOLDERS[activeId]

    useWindowNav(
        atMobileRoot ? null : { title: current.label, onBack: () => setFolder(null) },
        [atMobileRoot, current.label]
    )
    ...
    return (
        <div className="flex h-full font-inter">
            {!isMobile && (
                <aside className="w-32 shrink-0 overflow-auto border-r border-white/10 bg-black/25 px-2 py-3 sm:w-48">
                ...
                {!isMobile && (
                    <div className="flex shrink-0 items-center justify-between border-b border-white/10 px-4 py-2">
                        ...
                        <span className="text-xs text-white/35">
                            ...
                            {current.items.length === 1 ? 'item' : 'items'}
                        </span>
                    </div>
                )}
